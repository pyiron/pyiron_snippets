...
class TestResolvers(unittest.TestCase):
    ...
    def test_resource_resolver(self):
        ...
        self.assertEqual(
            res.first(), tuple(res.search())[0], "first does not return first result!"
        )
        self.assertEqual(list(res.search()), res.list(), "list not equal to search!")
        with self.assertRaises(
            ResourceNotFound, msg="first does not raise error on non existing resource!"
        ):
            res.first("nonexisting")
        res = ResourceResolver([self.res1, self.res2], "module3")
        self.assertTrue(
            len(res.list("empty.txt")) == 2,
    ...
    def test_order(self):
        ...
        self.assertTrue(
            "res1" in ResourceResolver([self.res1, self.res2], "module3").first(),
            "resolver does not respect order of given resource paths!",
        )
        self.assertTrue(
            "res2" in ResourceResolver([self.res2, self.res1], "module3").first(),
            "resolver does not respect order of given resource paths!",
        )
        self.assertEqual(
            tuple(
                os.path.basename(r)
                for r in ResourceResolver([self.res1], "module1").search()
            ),
    ...
    def test_chain(self):
        ...
        chain = ResourceResolver([self.res1], "module3").chain(
            ResourceResolver([self.res2], "module3")
        )
        ...
        self.assertIs(
            resol, resol.chain(), "Empty chain does not return the same resolver!"
        )

    def test_executable(self):
        for suffix in (None, "sh", "bat"):
            with self.subTest(suffix=suffix):
                res = ExecutableResolver(
                    [self.res1], code="code1", module="module1", suffix=suffix
                )
                ...
                res = ExecutableResolver(
                    [self.res1], code="code2", module="module1", suffix=suffix
                )
                self.assertEqual(
                    res.default_version,
                    "version2_default",
                ...
                self.assertEqual(
                    dict(res.search()), res.dict(), "dict not equal to search!"
                )
    ...
    def test_resource_resolver_subdirs(self):
        ...
        expected_results = {
            ...
            for path in (
                "run_code1_versionnonexec.sh",
                "run_code1_version1.sh",
                "run_code1_version2.sh",
            )
        }
        self.assertEqual(
            set(res.search("*code1*.sh")),
            expected_results,
